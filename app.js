...
function footballProfileCell(context, key) {
  ...
  const detail = Number.isFinite(percentile) ? `${percentile.toFixed(0)}th percentile` : "Sample developing";
  return `
...
function fullMatchupPreview(matchup) {
  ...
  const identityRows = [
    ...
    ["Offensive ADV Level", "rolling_off_adv_level", (value) => weeklyNumber(value, 1)],
    ["Defensive ADV Level", "rolling_def_adv_level", (value) => weeklyNumber(value, 1)],
    ["Weak-Side Profile", "rolling_weak_side_level", (value) => weeklyNumber(value, 1)],
  ...
  const statRows = [
    ["Games With Stats", "games_available", (value) => weeklyNumber(value, 0)],
    ...
    ["Turnover Margin", "turnover_margin", (value) => signed(value)],
  ...
  return `
    <section class="matchup-preview-detail">
      ...
      <p class="weekly-context-note">Pregame ADV Rating is the frozen opponent-adjusted strength view. ADV Schedule Rating is the model's raw opponent-strength rating, not a percentile. Positive TYI means performance above roster expectation; negative TYI means below expectation. Recent Form describes direction relative to the team's own season baseline.</p>
      ...
      <p class="weekly-context-note">${escapeHtml(away.comparison_stats?.coverage_note || "")} ${escapeHtml(home.comparison_stats?.coverage_note || "")}</p>
      <div class="weekly-profile-table matchup-preview-table">
        <div class="weekly-profile-row is-header">
